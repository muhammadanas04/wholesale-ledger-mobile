...
// Sub-component to render individual payment row items and resolve Customer relation reactively
function PaymentRow({ payment }: { payment: Payment }) {
  const customer = useRelation(payment.customer);
  ...
  return (
    <Pressable className="flex-row justify-between items-center bg-white dark:bg-slate-800 px-5 py-4 border-b border-slate-100 dark:border-slate-800/40 active:bg-slate-50 dark:active:bg-slate-700/30">
      ...
      <View className="items-end">
        <Text className="text-base font-mono font-bold text-emerald-600 dark:text-emerald-400">
          -{formatCurrency(payment.amount)}
        </Text>
        {payment.discount > 0 ? (
          <Text className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold mt-0.5">
            Disc: {formatCurrency(payment.discount)}
          </Text>
        ) : (
          <Text className="text-[10px] text-slate-300 dark:text-slate-600 uppercase font-bold mt-0.5">
            {payment.synced === 1 ? 'Synced' : 'Pending'}
          </Text>
        )}
      </View>
    </Pressable>
...
export default function PaymentsListScreen() {
  ...
  const onRefresh = async () => {
    ...
    try {
      ...
      console.error('Payments pull refresh failed:', e);
      Toast.show({
        type: 'error',
        text1: 'Sync Failed',
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50 dark:bg-slate-900">
        ...
        ListEmptyComponent={
          <View className="flex-1 items-center justify-center py-20 px-8">
            ...
            <Text className="text-slate-700 dark:text-slate-300 font-bold text-lg mt-4 text-center">
              {searchQuery ? 'No Results Found' : 'No Payments Recorded Yet'}
            </Text>
            <Text className="text-slate-400 dark:text-slate-500 text-sm mt-1 text-center max-w-[260px]">
              {searchQuery
                ...
                : 'Record payments using the plus button or pull down to sync from the ledger.'}
            </Text>
          </View>
        }
      />
      ...
      <Link href="/payments/new" asChild>
        <TouchableOpacity
